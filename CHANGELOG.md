...
- Resolve aggregate XML namespaces by element identity: equivalent default/prefixed RFC 7489 and RFC 9990 reports now parse consistently, extension declarations cannot mislabel the root namespace, and extension elements cannot supply DMARC fields. XML recovery diagnostics are retained alongside reporter-supplied errors.
...
- **Failure report fields retain folded continuation lines and correctly exclude CFWS comments from mechanism names.** Nested comments and escaped parentheses are handled without splitting commas inside comments; malformed mechanism syntax is rejected. MIME feedback decoding also preserves literal backslashes and trailing apostrophes in field values.
